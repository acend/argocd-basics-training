...
weight: 62
onlyWhen: application-sets
---

With the ApplicationSet ArgoCD adds support for managing ArgoCD Applications across a large number of clusters and environments. Plus it adds the capability of managing multi-tenant Kubernetes clusters.
...
* Within multi-tenant clusters, it improves the ability of individual cluster tenants to deploy applications using Argo CD (without needing to involve privileged cluster administrators in enabling the destination clusters/namespaces)
...
## {{% task %}} Create an ApplicationSet

For better understanding, let's create our first ApplicationSet. Create a yaml file with the following content under `~/argocd-training-examples/application-set/simple-example/application-set.yaml`.
...
Create the following `appset-application.yaml` and apply it:
...
```bash
{{% param cliToolName %}} apply -f appset-application.yaml
```
{{% /onlyWhen %}}
...
You even have the possibility to combine multiple generators together using the Matrix generator.
...
## {{% task %}} Matrix Example ApplicationSet

In this lab section we're going to create an ApplicationSet for a multi-environment.
...
kind: Application
metadata:
  name: argo-appset-matrix-<username>
  namespace: {{% param argoInfraNamespace %}}
  finalizers:
    - resources-finalizer.argocd.argoproj.io
spec:
